...
from . import Corpus, register_dataset
...
def PIBTEST_meta(split):
    ...
    for lang in langs:
        ...
        for src in [lang, "en"]:
            sub_path = "pib-test/{}/{}.{}".format(dirname, split, src)
...
def PIB_meta(split):
    ...
    for src, tgt in list(perm):
        for lang in [src, tgt]:
            sub_path = "pib/{}-{}/{}.{}".format(src, tgt, "train", lang)
...
def PIBV2_meta(split):
    ...
    for src, tgt in list(perm):
        for lang in [src, tgt]:
            sub_path = "pib-v1/{}-{}/{}.{}".format(src, tgt, "train", lang)
...
def PIBV02_meta(split):
    ...
    for src, tgt in list(perm):
        for lang in [src, tgt]:
            sub_path = "pib-v0.2/{}-{}/{}.{}".format(src, tgt, "train", lang)
...
def MKB_meta(split):
    ...
    for src, tgt in list(perm):
        for lang in [src, tgt]:
            sub_path = "mkb/{}-{}/{}.{}".format(src, tgt, "mkb", lang)
...
def WAT_MKB_DEV(split):
    ...
    for lang in langs:
        for src in [lang, "en"]:
            sub_path = "wat-mkb-dev/{}.{}-en.{}".format(split, lang, src)
...
def WAT_MKB_TEST(split):
    ...
    for lang in langs:
        for src in [lang, "en"]:
            sub_path = "wat-mkb-test/{}.{}-en.{}".format(split, lang, src)
...
def ILCI_meta(split):
    ...
    #    return []
    corpora = []
    langs = ["en", "te", "hi", "ml", "ta", "ud", "bg", "mr", "gj", "pj", "kn"]
