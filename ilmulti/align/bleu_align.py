...
class BLEUAlign:
    @staticmethod
    def withFiles(
        src: IO[str], tgt: IO[str], s2t: IO[str] = None, t2s: IO[str] = None
...
class BLEUAligner:
    def __init__(self, e2e_translator=None, tokenizer=None, splitter=None):
        if e2e_translator is None and tokenizer is None and splitter is None:
            raise RunTimeError(
                "You seem to have called BLEUAligner without any of e2e_translator, "
    ...
    def align(
        ...
        """
        return self._align(
            src, src_lang, tgt, tgt_lang, s2t, t2s, ssplit_src=False, ssplit_tgt=False
        )
    ...
    def _align(
        ...
        s2t: str,
        t2s: str = None,
        ssplit_src: bool = False,
        ssplit_tgt: bool = False,
    ) -> Tuple[List[str], List[str]]:
    ...
    def align_forward(self, src: str, src_lang: Lang, tgt: str, tgt_lang: Lang):
        ...
        """
        src, s2t = self._process_translation_result(
            self.e2e_translator(src, tgt_lang=tgt_lang, src_lang=src_lang)
        )
    ...
    def align_bidirectional(self, src: str, src_lang: Lang, tgt: str, tgt_lang: Lang):
        ...
        """
        src, s2t = self._process_translation_result(
            self.e2e_translator(src, tgt_lang=tgt_lang)
        )
        tgt, t2s = self._process_translation_result(
            self.e2e_translator(tgt, tgt_lang=src_lang)
        )
        return self._align(
            src,
            src_lang,
